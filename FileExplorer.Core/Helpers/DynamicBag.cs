...
public class DynamicBag<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
{
	...
	public DynamicBag(IEnumerable<T> collection)
	{
	...
	private bool TrySteal([MaybeNullWhen(false)] out T result, bool take)
	{
		while (true)
		{
			...
			var initialEmptyToNonEmptyCounts = Interlocked.Read(ref _emptyToNonEmptyListTransitionCount);
			...
			var gotItem = localQueue == null ? TryStealFromTo(_workStealingQueues, null, out result, take) : (TryStealFromTo(localQueue._nextQueue, null, out result, take) || TryStealFromTo(_workStealingQueues, localQueue, out result, take));
			if (gotItem)
			{
...
#pragma warning restore CS8762
	...
	public void CopyTo(T[] array, int index)
	{
		...
		{
			throw new ArgumentException("Too many elements", nameof(index));
		}

		// Do the copy
		try
		{
			var copied = CopyFromEachQueueToArray(array, index);
		}
	...
	public int CopyFromEachQueueToArray(T[] array, int index)
	{
		var i = index;
		for (var queue = _workStealingQueues; queue != null; queue = queue._nextQueue)
		{
	...
	public void FreezeBag(ref bool lockTaken)
	{
	...
	public void UnfreezeBag(bool lockTaken)
	{
		if (lockTaken)
		{
	...
	private sealed class WorkStealingQueue
	{
		...
		internal void LocalPush(T item, ref long emptyToNonEmptyListTransitionCount)
		{
			...
			try
			{
				...
				var head = _headIndex; // read after _currentOp set to Add
				...
				else
				{
					...
					head = _headIndex;
					var count = tail - head; // this count is stable, as we're holding the lock
		...
		internal bool TrySteal([MaybeNullWhen(false)] out T result, bool take)
		{
			lock (this)
			{
				var head = _headIndex; // _headIndex is only manipulated under the lock
				if (take)
				{
		...
		public int DangerousCopyTo(T[] array, int arrayIndex)
		{
			var headIndex = _headIndex;
			var count = DangerousCount;

			// Copy from this queue's array to the destination array, but in reverse
			// order to match the ordering of desktop.
			for (var i = arrayIndex + count - 1; i >= arrayIndex; i--)
			{
		...
		internal int DangerousCount
		{
			get
			{
				var stealCount = _stealCount;
				var addTakeCount = _addTakeCount;
				var count = addTakeCount - stealCount;
				return count;
			}
		}
	}

...
public class DynamicQueue<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
{
	...
	public DynamicQueue(IEnumerable<T> collection)
	{
		...
		var length = InitialSegmentLength;
	...
	{
		get { throw new NotSupportedException("SyncRoot is not supported"); }
	}
	...
	public T[] ToArray()
	{
		...
		var count = GetCount(head, headHead, tail, tailTail);
		var arr = new T[count];
		var i = 0;

		// Now enumerate the contents, copying each element into the array.
		foreach (var item in Enumerate(head, headHead, tail, tailTail))
		{
			arr[i++] = item;
		}
	...
	private static long GetCount(ConcurrentQueueSegment<T> head, int headHead, ConcurrentQueueSegment<T> tail, int tailTail)
	{
		...
		if (headHead < headTail)
		{
			...
			count += headHead < headTail ? headTail - headHead : head._slots.Length - headHead + headTail;
		}
	...
	public void AppendTo(DynamicList<T> list)
	{
		...
		lock (_crossSegmentLock)
		{
			...
			list.EnsureCapacity(list.Count + (int)GetCount(head, headHead, tail, tailTail));

			// Copy the items to the target array
			list.AddRange(Enumerate(head, headHead, tail, tailTail));
	...
	{
		SnapForObservation(out var head, out var headHead, out var tail, out var tailTail);
		return Enumerate(head, headHead, tail, tailTail).GetEnumerator();
	}
	...
	private static T GetItemWhenAvailable(ConcurrentQueueSegment<T> segment, int i)
	{
		...
		var expectedSequenceNumberAndMask = (i + 1) & segment._slotsMask;
	...
	private static IEnumerable<T> Enumerate(ConcurrentQueueSegment<T> head, int headHead, ConcurrentQueueSegment<T> tail, int tailTail)
	{
		Debug.Assert(head._preservedForObservation);
		Debug.Assert(head._frozenForEnqueues);
		Debug.Assert(tail._preservedForObservation);
		...
		if (headHead < headTail)
		{
			...
			if (headHead < headTail)
			{
				for (var i = headHead; i < headTail; i++) yield return GetItemWhenAvailable(head, i);
			}
			else
			{
				for (var i = headHead; i < head._slots.Length; i++) yield return GetItemWhenAvailable(head, i);
				for (var i = 0; i < headTail; i++) yield return GetItemWhenAvailable(head, i);
			}
		}
		...
		if (head != tail)
		{
			...
			for (var s = head._nextSegment!; s != tail; s = s._nextSegment!)
			{
				...
				var sTail = s._headAndTail.Tail - s.FreezeOffset;
				for (var i = 0; i < sTail; i++)
				{
		...
		while (true)
		{
			...
			lock (_crossSegmentLock)
			{
				if (tail == _tail)
				{
					...
					var nextSize = tail._preservedForObservation ? InitialSegmentLength : Math.Min(tail.Capacity * 2, MaxSegmentLength);

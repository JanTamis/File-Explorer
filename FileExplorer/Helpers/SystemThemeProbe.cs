...
public static partial class SystemThemeProbe
{
	/// <summary>
	...
	[LibraryImport("advapi32.dll", EntryPoint = "RegOpenKeyEx", StringMarshalling = StringMarshalling.Utf16)]
	private static partial int RegOpenKeyEx_DllImport(UIntPtr hKey, string lpSubKey, uint ulOptions, int samDesired, out UIntPtr phkResult);

	[LibraryImport("advapi32.dll", EntryPoint = "RegQueryValueEx", StringMarshalling = StringMarshalling.Utf16)]
	private static partial int RegQueryValueEx_DllImport(UIntPtr hKey, string lpValueName, int lpReserved, out uint lpType, byte[] lpData, ref int lpcbData);
	...
	public static BaseThemeMode GetWindowsBaseThemeMode()
	{
		...
		if (o0 != 0)
		{
			throw new Exception("Something went wrong when opening \"Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize\" registry entry");
		}
		...
		if (o1 != 0)
		{
			throw new Exception("Something went wrong when reading \"AppsUseLightTheme\" registry entry value");
		}
